...
### 4. “我的”模块
- **用户信息**：
  - 个人资料编辑与展示：用户名称（不需要编辑上传头像，使用默认头像）
  - 身体数据：性别、体重、身高
  - 运动偏好设置：是否有训练习惯

- **设置**:
  - 创建SettingsView.swift文件
  - 实现账户安全设置（手机号绑定、解绑）
  - 实现隐私设置（数据分享权限）
  - 实现Apple健康集成开关
  - 实现通知设置
...
- **关于**：
  - 创建AboutView.swift文件
  - 添加隐私政策、协议条款内容（可以是WebView加载）
  - 添加关于DessertRun信息（版本号、团队信息）
  - 添加反馈与建议提交表单

### 5. 登陆与注册
- **启动引导页**
  - 创建OnboardingView.swift作为应用首次启动的引导页
  - 先做一个简洁明了的，放一个图像占位即可

- **登录注册页**
  - 创建LoginView.swift作为主登录页面
  - 实现手机号一键登录服务
  - 实现微信授权登录按钮
  - 实现Apple ID登录选项

- **初次登录信息收集**
  - 创建UserInfoSetupView.swift
  - 设计表单引导流程，分步收集：
  - 步骤1：基本信息（姓名、性别）
  - 步骤2：身体数据（身高、体重）
  - 步骤3：运动习惯（运动频率、偏好）
...
- **不同用户的不同登录体验**
  - 新用户首次登陆：启动页 - 登录/注册页 - 个人信息收集 - 注册成功 - 关闭登录注册流程进入首页
  - 老用户登录：启动页 - 登录/注册页 - 登录成功 - 关闭登录注册流程进入首页
  - 老用户已登录状态：启动页 - 进入首页
